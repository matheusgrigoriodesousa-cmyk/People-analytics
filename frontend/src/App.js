...
function App() {
  ...
  // Hook de funcionários consumindo a API do Render
  const { 
    employees, 
    dashboardData, 
    departments, 
  ...
  useEffect(() => {
    ...
    if (token && userName) {
      ...
      // Não chamamos fetchData aqui para evitar loops, 
      // deixamos o useEffect abaixo cuidar disso quando o 'user' for setado.
    }
  }, []);

  useEffect(() => {
    // Só busca dados se houver usuário e o servidor não estiver carregando
    if (user && !loading && employees.length === 0 && !error) {
      fetchData();
    }
  }, [user, fetchData]); // fetchData deve estar em um useCallback no hook
  ...
  const handleSave = async (data) => {
    ...
    if (success) fetchData(); // Garante atualização após salvar
  };
    ...
    <div className="dashboard-container">
      ...
      <div className="dashboard-controls no-print">
        ...
        <div className="toolbar-row" style={{marginBottom: '20px'}}>
           {currentView === 'users' ? (
              <button className="btn-action" onClick={() => setCurrentView('dashboard')}>
            ...
            <div className="actions-group">
                {user.role === 'admin' && currentView === 'dashboard' && (
                  <button className="btn-action btn-admin" onClick={() => setCurrentView('users')}>
